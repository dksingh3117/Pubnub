! function(a, b) {
	...
	return function(a) {
		...
		function g(a, b) {
			var c = [];
			return r.each(a.split(/\s+/), function(a) {
				r.each((b || document).getElementsByTagName(a), function(a) {
					c.push(a)
				})
			}), c
		}
		...
		function m(a) {
			...
			try {
				...
				}, b.onload = b.onloadend = l;
				var m = r.build_url(a.url, h);
		...
		var t = function() {
			...
			u = {
				...
				bind: function(a, b) {
					(u.list[a] = u.list[a] || []).push(b)
				},
				fire: function(a, b) {
					r.each(u.list[a] || [], function(a) {
						a(b)
					})
				}
			},
			v = function(a) {
				...
				a.xdr = m, a.db = t, a.error = a.error || f, a._is_online = n, a.hmac_SHA256 = d, a.crypto_obj = p(), a.sendBeacon = o, a.sdk_family = "Web";
				var c = function(a) {
						return v(a)
					},
					q = r.PN_API(a);
				for (var w in q) q.hasOwnProperty(w) && (c[w] = q[w]);
				return c.css = k, c.$ = e, c.create = l, c.bind = h, c.head = i, c.search = g, c.attr = j, c.events = u, c.init = c, c.secure = c, c.crypto_obj = p(), c.WS = s, c.PNmessage = r.PNmessage, c.supplant = r.supplant, h("beforeunload", window, function() {
					return b && c["each-channel"](function(a) {
						c.LEAVE(a.name, 0)
					}), !0
				}), c.ready(), a.notest ? c : (h("offline", window, c.offline), h("offline", document, c.offline), c)
			};
		v.init = v, v.secure = v, v.crypto_obj = p(), v.WS = s, v.db = t, v.PNmessage = r.PNmessage, v.uuid = r.uuid, v.css = k, v.$ = e, v.create = e, v.bind = h, v.head = i, v.search = g, v.attr = j, v.events = u, v.map = r.map, v.each = r.each, v.grep = r.grep, v.supplant = r.supplant, v.now = r.now, v.unique = r.unique, v.updater = r.updater, window.jQuery && (window.jQuery.PUBNUB = v), a.exports = v
		...
		function i(a) {
			...
			function c(a) {
				return a || (a = {}), l.each(va, function(b, c) {
					b in a || (a[b] = c)
				}), a.pnsdk = b(), a
			}
			...
			function m(a, b) {
				return Ja.encrypt(a, b || Aa) || a
			}

			function r(a, b) {
				return Ja.decrypt(a, b || Aa) || Ja.decrypt(a, Aa) || a
			}

			function z() {
				return clearTimeout(oa), !qa || qa >= 500 || qa < 1 || !e(la, !0).length && !f(ma, !0).length ? void(ra = !1) : (ra = !0, void Ka.presence_heartbeat({
					callback: function(a) {
						oa = l.timeout(z, qa * u)
					},
					error: function(a) {
						wa && wa("Presence Heartbeat unable to reach Pubnub servers." + JSON.stringify(a)), oa = l.timeout(z, qa * u)
					}
				}))
			}
			...
			function B(a) {
				if (sa) {
					...
					if (a && (ca.sending = 0), ca.sending || !ca.length) return;
					ca.sending = 1
				}
				G(ca.shift())
			}
			...
			function G(a) {
				var b = a.operation,
					...
					d = a.data || {};
				if (X) {
					d.auth || delete d.auth, d.timestamp = c;
					var e = V + "\n" + U + "\n";
					...
					e += j(d);
					var g = Y(e, X);
					g = g.replace(/\+/g, "-"), g = g.replace(/\//g, "_"), d.signature = g, a.data = d
				}
				return ua(a)
			}
			...
			function H(a, b, d, e) {
				var b = a.callback || b,
					f = a.error || wa,
					g = ya();
				e = e || {}, e.auth || (e.auth = a.auth_key || W);
				var h = [_, "v1", "channel-registration", "sub-key", V];
				h.push.apply(h, d), g && (e.callback = g), G({
					callback: g,
					data: c(e),
					success: function(a) {
						E(a, b, f)
			...
			function I() {
				...
				}), N && clearTimeout(N), N = l.timeout(I, u)
			}

			function J() {
				S && Ka.time(function(a) {
					M(function() {}, a), a || K(1, {
						error: "Heartbeat failed to connect to Pubnub Servers.Please check your network settings."
					}), O && clearTimeout(O), O = l.timeout(J, R)
				})
			}

			function K(a, b) {
				ha && ha(a, b), ha = null, clearTimeout(N), clearTimeout(O)
			}
			...
			function M(a, b) {
				function c(b) {
					if (b) {
						var c = b / 1e4,
							e = (l.rnow() - d) / 2;
						ea = l.rnow() - (c + e), a && a(ea)
					}
				}
				var d = l.rnow();
				b && c(b) || Ka.time(c)
			}
			var N, O, P = +a.windowing || q,
				Q = (+a.timeout || s) * u,
				R = (+a.keepalive || t) * u,
				S = a.timecheck || 0,
				T = a.noleave || 0,
				...
				Y = a.hmac_SHA256,
				Z = a.ssl ? "s" : "",
				$ = "http" + Z + "://" + (a.origin || "pubsub.pubnub.com"),
				_ = y($),
				aa = y($),
				ba = function() {},
				ca = [],
				...
				na = {},
				oa = null,
				pa = k(a.heartbeat || a.pnexpires || 0, a.error),
				qa = a.heartbeat_interval || pa / 2 - 1,
				ra = !1,
				sa = a.no_wait_for_pending,
				ta = a["compatible_3.5"] || !1,
				ua = a.xdr,
				va = a.params || {},
				wa = a.error || function() {},
				xa = a._is_online || function() {
					return 1
				},
				ya = a.jsonp_cb || function() {
					return 0
				},
				za = a.db || {
					get: function() {},
				...
				Aa = a.cipher_key,
				Ba = a.uuid || !a.unique_uuid && za && za.get(V + "uuid") || "",
				Ca = a.instance_id || !1,
				Da = a.partner_id,
				Ea = a.sdk_family,
				Fa = "",
				Ga = a.shutdown,
				Ha = "undefined" == typeof a.use_send_beacon || a.use_send_beacon,
				Ia = Ha ? a.sendBeacon : null;
			2 === pa && (qa = 1);
			var Ja = a.crypto_obj || {
				...
				Ka = {
					LEAVE: function(a, b, d, e, f) {
						...
						if (T) return !1;
						if ("0" != m && (i.callback = m), Ca && (i.instanceid = Fa), g = [j, "v2", "presence", "sub_key", V, "channel", l.encode(a), "leave"], h = c(i), Ia) {
					...
					LEAVE_GROUP: function(a, b, d, e, f) {
						var g, h, i = {
								uuid: Ba,
								auth: d || W
							},
						...
						if (T) return !1;
						if ("0" != m && (i.callback = m), a && a.length > 0 && (i["channel-group"] = a), Ca && (i.instanceid = Fa), g = [j, "v2", "presence", "sub_key", V, "channel", l.encode(","), "leave"], h = c(i), Ia) {
							var n = l.buildURL(g, h);
							if (Ia(n)) return e && e({
								status: 200,
								action: "leave",
					...
					set_heartbeat: function(a, b) {
						pa = k(a, pa, wa), qa = b || pa / 2 - 1, 2 == pa && (qa = 1), ba(), z()
					},
					...
					channel_group: function(a, b) {
						...
						c && h.push("namespace") && h.push(l.encode(c)), h.push("channel-group"), d && "*" !== d && h.push(d), f ? (l.isArray(f) && (f = f.join(",")), i[j] = f, i.cloak = da ? "true" : "false") : "remove" === j && h.push("remove"), "undefined" != typeof g && (i.cloak = g ? "true" : "false"), H(a, b, h, i)
					},
					channel_group_list_groups: function(a, b) {
						var c;
						c = a.namespace || a.ns || a.channel_group || null, c && (a.channel_group = c + ":*"), Ka.channel_group(a, b)
					},
					channel_group_list_channels: function(a, b) {
						return a.channel_group ? void Ka.channel_group(a, b) : wa("Missing Channel Group")
					},
					channel_group_remove_channel: function(a, b) {
						return a.channel_group ? a.channel || a.channels ? (a.mode = "remove", void Ka.channel_group(a, b)) : wa("Missing Channel") : wa("Missing Channel Group")
					},
					channel_group_remove_group: function(a, b) {
						return a.channel_group ? a.channel ? wa("Use channel_group_remove_channel if you want to remove a channel from a group.") : (a.mode = "remove", void Ka.channel_group(a, b)) : wa("Missing Channel Group")
					},
					channel_group_add_channel: function(a, b) {
						return a.channel_group ? a.channel || a.channels ? void Ka.channel_group(a, b) : wa("Missing Channel") : wa("Missing Channel Group")
					},
					channel_group_cloak: function(a, b) {
						return "undefined" == typeof a.cloak ? void b(da) : (da = a.cloak, void Ka.channel_group(a, b))
					},
					...
					history: function(a, b) {
						var b = a.callback || b,
							...
							m = a.end,
							n = a.include_token,
							o = a.string_message_token || !1,
							p = {},
							q = ya();
						return i || j ? b ? V ? (p.stringtoken = "true", p.count = d, p.reverse = e, p.auth = g, j && (p["channel-group"] = j, i || (i = ",")), q && (p.callback = q), k && (p.start = k), m && (p.end = m), n && (p.include_token = "true"), o && (p.string_message_token = "true"), void G({
							callback: q,
							data: c(p),
							success: function(a) {
								...
								for (var c = a[0], d = [], e = 0; e < c.length; e++)
									if (n) {
										var g = r(c[e].message, h),
											i = c[e].timetoken;
										try {
											d.push({
							...
							url: [_, "v2", "history", "sub-key", V, "channel", l.encode(i)]
						})) : wa("Missing Subscribe Key") : wa("Missing Callback") : wa("Missing Channel")
					},
					replay: function(a, b) {
						var d, b = b || a.callback || function() {},
							e = a.auth_key || W,
							f = a.source,
							g = a.destination,
							...
							n = ya(),
							o = {};
						return f ? g ? U ? V ? ("0" != n && (o.callback = n), i && (o.stop = "all"), l && (o.reverse = "true"), j && (o.start = j), k && (o.end = k), m && (o.count = m), o.auth = e, d = [_, "v1", "replay", U, V, f, g], void G({
							callback: n,
							success: function(a) {
								E(a, b, h)
							},
							...
							url: d,
							data: c(o)
						})) : wa("Missing Subscribe Key") : wa("Missing Publish Key") : wa("Missing Destination Channel") : wa("Missing Source Channel")
					},
					auth: function(a) {
						W = a, ba()
					},
					...
					publish: function(a, b) {
						var d = a.message;
						if (!d) return wa("Missing Message");
						var e, f, b = b || a.callback || d.callback || a.success || function() {},
							...
							q = ya(),
							r = "push";
						return a.prepend && (r = "unshift"), g ? U ? V ? (d.getPubnubMessage && (d = d.getPubnubMessage()), d = JSON.stringify(m(d, j)), f = [_, "publish", U, V, 0, l.encode(g), q, l.encode(d)], e = {
							uuid: Ba,
							auth: i
						}, h && "object" == typeof h && (e.meta = JSON.stringify(h)), o || (e.store = "0"), p || (e.norep = "true"), Ca && (e.instanceid = Fa), ca[r]({
							callback: q,
							url: f,
							data: c(e),
							fail: function(a) {
							...
							mode: n ? "POST" : "GET"
						}), void B()) : wa("Missing Subscribe Key") : wa("Missing Publish Key") : wa("Missing Channel")
					},
					fire: function(a, b) {
						a.store_in_history = !1, a.replicate = !1, Ka.publish(a, b)
					},
					unsubscribe: function(a, b) {
						var c = a.channel,
							...
							f = a.error || function() {};
						if (!c && !d) return wa("Missing Channel or Channel Group");
						if (!V) return wa("Missing Subscribe Key");
						if (c) {
							...
							l.each(h, function(a) {
								...
								a in la && delete la[a], a in na && delete na[a]
							}), 0 === la.length && 0 === ma.length && (ja = 0);
							var j = !0;
							n && (j = Ka.LEAVE(h.join(","), 0, e, b, f)), j || b({
								action: "leave"
							})
						}
						if (d) {
							...
							l.each(m, function(a) {
								...
								a in ma && delete ma[a], a in na && delete na[a]
							}), 0 === la.length && 0 === ma.length && (ja = 0);
							var j = !0;
							n && (j = Ka.LEAVE_GROUP(m.join(","), 0, e, b, f)), j || b({
								action: "leave"
							})
						}
						ba()
					...
					subscribe: function(a, b) {
						function d(a) {
							a ? l.timeout(ba, B) : (_ = y($), aa = y($), l.timeout(function() {
								Ka.time(d)
							}, u)), D(function(b) {
						...
						function g() {
							...
							if (b || h) {
								...
								h && (i["channel-group"] = h);
								var j = JSON.stringify(na);
								j.length > 2 && (i.state = JSON.stringify(na)), pa && (i.heartbeat = pa), Ca && (i.instanceid = Fa), A(), ha = G({
									timeout: z,
									callback: a,
									fail: function(a) {
										a && a.error && a.service ? (F(a, q), d(!1)) : Ka.time(function(b) {
											!b && F(a, q), d(b)
										})
									},
									data: c(i),
									url: [aa, "subscribe", V, l.encode(b), a, ja],
									success: function(a) {
										if (!a || "object" == typeof a && "error" in a && a.error) return q(a), l.timeout(ba, u);
										if (s(a[1]), ja = !ja && ia && za.get(V) || a[1], D(function(a) {
												a.connected || (a.connected = 1, a.connect(a.name))
											}), C(function(a) {
												a.connected || (a.connected = 1, a.connect(a.name))
											}), ka && !ia) return ja = 0, ka = !1, za.set(V, 0), void l.timeout(g, B);
										w && (ja = 1e4, w = 0), za.set(V, a[1]);
										var b = function() {
												var b = "",
													c = "";
												a.length > 3 ? (b = a[3], c = a[2]) : b = a.length > 2 ? a[2] : l.map(e(la), function(b) {
												...
												return function() {
													...
													b ? (a && a.indexOf("-pnpres") >= 0 && b.indexOf("-pnpres") < 0 && (b += "-pnpres"), c = ma[b] || la[b] || e) : c = la[a] || e;
													var g = [c.callback || fa, a.split(p)[0]];
													return b && g.push(b.split(p)[0]), g
												}
											}(),
											c = L(+a[1]);
										l.each(a[0], function(d) {
											var e = b(),
												f = r(d, la[e[1]] ? la[e[1]].cipher_key : null);
						...
						var h = a.channel,
							...
							m = a.disconnect || function() {},
							q = a.error || q || function() {},
							s = a.idle || function() {},
							t = a.presence || 0,
							v = a.noheresync || 0,
							w = a.backfill || 0,
							...
							B = a.windowing || P,
							E = a.state,
							H = a.heartbeat || a.pnexpires,
							I = a.heartbeat_interval,
							J = a.restore || ia;
						return W = a.auth_key || W, ia = J, ja = x, h || i ? b ? V ? ((H || 0 === H || I || 0 === I) && Ka.set_heartbeat(H, I), h && l.each((h.join ? h.join(",") : "" + h).split(","), function(d) {
							var e = la[d] || {};
							la[ga = d] = {
								name: d,
								...
								disconnect: m,
								reconnect: k
							}, E && (d in E ? na[d] = E[d] : na[d] = E), t && (Ka.subscribe({
								channel: d + p,
								callback: t,
								restore: J
							}), e.subscribed || v || Ka.here_now({
								channel: d,
								data: c({
									uuid: Ba,
									auth: W
							...
							K(), l.timeout(g, B)
						}, n ? void ba() : o.push(ba)) : wa("Missing Subscribe Key") : wa("Missing Callback") : wa("Missing Channel")
					},
					here_now: function(a, b) {
						...
						if (j || (m.disable_uuids = 1), k && (m.state = 1), !b) return wa("Missing Callback");
						if (!V) return wa("Missing Subscribe Key");
						var n = [_, "v2", "presence", "sub_key", V];
						g && n.push("channel") && n.push(l.encode(g)), "0" != i && (m.callback = i), h && (m["channel-group"] = h, !g && n.push("channel") && n.push(",")), Ca && (m.instanceid = Fa), G({
							callback: i,
							data: c(m),
							success: function(a) {
								E(a, b, e)
					...
					where_now: function(a, b) {
						...
						return b ? V ? ("0" != f && (h.callback = f), Ca && (h.instanceid = Fa), void G({
							callback: f,
							data: c(h),
							success: function(a) {
								E(a, b, d)
							...
							url: [_, "v2", "presence", "sub_key", V, "uuid", l.encode(g)]
						})) : wa("Missing Subscribe Key") : wa("Missing Callback")
					},
					state: function(a, b) {
						var d, b = a.callback || b || function(a) {},
							e = a.error || function() {},
							f = a.auth_key || W,
							g = ya(),
							h = a.state,
						...
						return V ? i ? j || k ? ("0" != g && (m.callback = g), "undefined" != typeof j && la[j] && la[j].subscribed && h && (na[j] = h), "undefined" != typeof k && ma[k] && ma[k].subscribed && (h && (na[k] = h), m["channel-group"] = k, j || (j = ",")), m.state = JSON.stringify(h), Ca && (m.instanceid = Fa), d = h ? [_, "v2", "presence", "sub-key", V, "channel", j, "uuid", i, "data"] : [_, "v2", "presence", "sub-key", V, "channel", j, "uuid", l.encode(i)], void G({
							...
							url: d
						})) : wa("Missing Channel") : wa("Missing UUID") : wa("Missing Subscribe Key")
					},
					grant: function(a, b) {
						var b = a.callback || b,
							d = a.error || function() {},
							...
							j = a.write ? "1" : "0",
							k = a.manage ? "1" : "0",
							m = a.auth_key || a.auth_keys;
						if (!b) return wa("Missing Callback");
						if (!V) return wa("Missing Subscribe Key");
						if (!U) return wa("Missing Publish Key");
						if (!X) return wa("Missing Secret Key");
						var n = {
							w: j,
							r: i
						};
						a.manage && (n.m = k), l.isArray(e) && (e = e.join(",")), l.isArray(m) && (m = m.join(",")), "undefined" != typeof e && null != e && e.length > 0 && (n.channel = e), "undefined" != typeof f && null != f && f.length > 0 && (n["channel-group"] = f), "0" != g && (n.callback = g), (h || 0 === h) && (n.ttl = h), m && (n.auth = m), n = c(n), m || delete n.auth, G({
					...
					mobile_gw_provision: function(a) {
						var b, c, d = a.callback || function() {},
							...
							j = a.gw_type,
							k = a.device_id;
						return k ? j ? i ? h ? V ? (c = [_, "v1/push/sub-key", V, "devices", k], b = {
							uuid: Ba,
							auth: e,
							type: j
						}, "add" == i ? b.add = h : "remove" == i && (b.remove = h), Ca && (b.instanceid = Fa), void G({
							callback: g,
							data: b,
							success: function(a) {
								E(a, d, f)
							...
							url: c
						})) : wa("Missing Subscribe Key") : wa("Missing gw destination Channel (channel)") : wa("Missing GW Operation (op: add or remove)") : wa("Missing GW Type (gw_type: gcm or apns)") : wa("Missing Device ID (device_id)")
					},
					audit: function(a, b) {
						var b = a.callback || b,
							d = a.error || function() {},
						...
						if (!X) return wa("Missing Secret Key");
						var i = {};
						"0" != h && (i.callback = h), "undefined" != typeof e && null != e && e.length > 0 && (i.channel = e), "undefined" != typeof f && null != f && f.length > 0 && (i["channel-group"] = f), g && (i.auth = g), i = c(i), g || delete i.auth, G({
							...
							url: [_, "v1", "auth", "audit", "sub-key", V]
						})
					},
					revoke: function(a, b) {
						a.read = !1, a.write = !1, Ka.grant(a, b)
					},
					set_uuid: function(a) {
						Ba = a, ba()
					},
					...
					presence_heartbeat: function(a) {
						var b = a.callback || function() {},
							...
							i = JSON.stringify(na);
						i.length > 2 && (h.state = JSON.stringify(na)), pa > 0 && pa < 320 && (h.heartbeat = pa), "0" != g && (h.callback = g);
						var j = l.encode(e(la, !0).join(",")),
							k = f(ma, !0).join(",");
						j || (j = ","), k && (h["channel-group"] = k), Ca && (h.instanceid = Fa), G({
							callback: g,
							data: c(h),
							url: [_, "v2", "presence", "sub-key", V, "channel", j, "heartbeat"],
							success: function(a) {
								E(a, b, d)
							},
							fail: function(a) {
					...
					stop_timers: function() {
						clearTimeout(N), clearTimeout(O), clearTimeout(oa)
					},
					shutdown: function() {
						Ka.stop_timers(), Ga && Ga()
					},
					xdr: ua,
					ready: g,
					db: za,
		...
		var j = c(5),
			...
			y = function() {
				...
				return function(c) {
					var d = c.split("://")[0],
						e = c.split("://")[1];
					return e.match("^ps") ? d + "://" + e.replace("ps", "ps" + (++b < a ? b : b = 1)) : e.match("^pubsub") ? d + "://" + e.replace("pubsub", "ps" + (++b < a ? b : b = 1)) : c
				}
			}();
		a.exports = {
			PN_API: i,
